...
float threshold = 2.5;
float v = 0.000;
int mode = 0; // 0 = Early mission stuff, 2 = standby mode, 4 = arming mode, 8 = deployment mode

/* 
 *  larger mission variables to control main loop
 */
...
void loop() 
{
  ...
  if(emo_mode)
  {
    if(!did_init_emo)
    {
      SerialUSB.println("Setting up emo mode");
      setup_emo_mode();
    }

    // this is the flight code that will make the SC hold 45 mins --> deploy the antennae
    loop_emo_mode();
    SerialUSB.println("In emo mode");

    if(rand() % 5 == 0)
    {
      SerialUSB.println("Antenna has been deployed");
  ...
  else if(deployment_mode)
  {
    ...
    {
      SerialUSB.println("Setting up deployment mode");
      setup_deployment_mode();
    }

    // this is the flight code that will make the mothership collect data, beacon, and all other wonderful stuff!
    SerialUSB.println("In deployment mode");
    loop_deployment_mode();

    if(rand() % 5 == 0)
    {
      SerialUSB.println("Sprites have been deployed");
      sprites_did_deploy = true;
    }
