...
void KickSat_Sensor::operate() {  
  ...
  if (board== "xtb1"){ //H.ALPERT devices
    ...
    dataOut[5] = readPins(0xCC, 0xFF, 0x80, 20, 1, 0x01);
     GPIO(0x00, 0x01);
    dataOut[6] = readPins(0x58, 0xF5, 0x80, 200, 100, 0x01);
     GPIO(0x00, 0x00);
     delay(50);
     GPIO(0x00, 0x02);
    dataOut[7] = readPins(0x59, 0xF5, 0x80, 200, 100, 0x01);
     GPIO(0x00, 0x00);
     delay(50);
     GPIO(0x00, 0x04);
    dataOut[8] = readPins(0x0A, 0xF0, 0x80, 200, 100, 0x01);
     GPIO(0x00, 0x00);
     delay(50);
     GPIO(0x00, 0x08);
    dataOut[9] = readPins(0x0B, 0xF0, 0x80, 200, 100, 0x01);
     GPIO(0x00, 0x00);
     datafile.write((const uint8_t *)&dataOut, sizeof(dataOut)); //save data to SD card as bytes (4 bytes per float);    
