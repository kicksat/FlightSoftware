/* seperated RH_RF22 from ax25 library
ax25_sans_radiohead.cpp
*/
...
// NEW: takes in a message, and sets member variable "finalSequence" with what you need to transmit
  ...
  */
void AX25::prepareToSend(char* message1)
{
  // sets the size of the message
  uint16_t = sizeof(message1);
  ...
  // copies message1 to message
  memcpy(message, message1, size);
  
  // format packet will call bit processing... 
  // bit processing does all the dirty work and makes your message into an AX25 packet!!
  // it stores this packet in the member variable final sequence.
  formatPacket(size);

  /* with your radiohead object, make sure to do the following
  radio.setModeIdle();
...
AX25::AX25(uint8_t slaveSelectPin, uint8_t interruptPin, uint8_t _shutdownPin)
{
  SerialUSB.println("Constructing your constructor so you can access functions and member variables!!");
}
