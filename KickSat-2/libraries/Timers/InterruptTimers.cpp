...
void TC5_Handler() {
  ...
  for (size_t i = 0; i < numberOfTimers; i++) {
    ...
    if (timerLastInterrupt[i] > 4294967295) {timerLastInterrupt[i] = 4294967295;} // Catch unexpected values for last interrupt time
    ...
    if (overflowed[i]) { // Handle overflowed counters
      ...
      } else { // transition away from overflow control, back to nominal case
      ...
      if (timerCounters[i] < 4294967295) { // overflow value for 32 bit unsigned integers, nominal case
        timerCounters[i]++; // Increment counter
        ...
        timerOverflow[i] = 4294967295-timerLastInterrupt[i]; // Set overflow counter
        timerCounters[i] = 1; // Reset counter
  ...
  // WatchDog here
  SerialUSB.println("WatchDog Here!");
